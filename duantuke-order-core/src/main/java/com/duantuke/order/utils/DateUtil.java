...
//            Date sDate = getDateFromString(second, FORMAT_DATE);
    ...
    public static double getDiffHoure(String beginTime, String endTime) {
        double dif = 0.00;
        try {
            Date eDatetime = DateUtil.getDateFromString(endTime, "yyyy-MM-dd HH:mm:ss");
            Date bDatetime = DateUtil.getDateFromString(beginTime, "yyyy-MM-dd HH:mm:ss");
    ...
    public static List<Date> listDays(Date begintime, Date endtime) throws Exception {
        ...
        for (int i = 0; i < daysnum; i++) {
            list.add(addDateOneDay(begintime, i));
        }
        return list;
    }
    
    private static Date addDateOneDay(Date date, int inday) {
        if (null == date) {
            return date;
        }
        Calendar c = Calendar.getInstance();
